...
**Twelve HIGH-priority items from the inspiration backlog land in a single sweep: memory tiers / edges-graph / HNSW, gateway budget meter + per-request traces, prompt registry, context7 doc fetcher, repo-map + signature extractor, `rtrt discover`, handlebars templating, rule-pass extensions + LLM compression mode.**
