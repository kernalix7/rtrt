...
- **안전한 설치 시 OpenCode machine bootstrap**: eligible non-root Linux/WSL 설치는 exact installed RTRT로 installer cwd를 발견·승인하지 않고 strict machine shell ownership을 생성·검증한 뒤 global session을 lossless migration합니다. `--no-setup`은 이 흐름과 Claude refresh를 모두 끄며 uninstall은 binary 삭제 전 prior shell을 복원하고 session DB를 보존합니다.

### 추가

- **안전한 dashboard browser 자동 인증**: `rtrt service open`은 현재 project의 private `dashboard.env`만 읽고 고정 loopback dashboard를 확인한 뒤 URL fragment에 60초 one-time HMAC-SHA256 bootstrap credential만 넣어 고정 OS browser opener를 argv로 실행합니다. SPA는 exchange 전에 fragment를 지우고 반환된 bearer를 `sessionStorage`에만 보관합니다. 일반 API는 계속 bearer-protected이며 정확한 POST exchange만 fixed Origin, strict bounded JSON, no-store response, time/MAC 검증, bounded replay 방지를 적용합니다.
- **Project-private OpenCode session 보존**: cwd와 무관한 `rtrt opencode sessions status|dry-run|apply`를 추가했습니다. Global DB를 WAL-safe read-only로 발견하고 canonical project별 graph로 migration하며 귀속 불가 session은 private `legacy-global`에 보존합니다. 민감한 control row 제외, atomic/idempotent conflict-safe write, launch 전 incremental catch-up을 포함합니다. Prompt-history JSONL은 계속 별도 보존합니다.

- **보안·compliance 정정**: 엄격한 Linux OpenCode shell confinement는 setup 소유이며 VM이 아닙니다. Sandbox 없는 생성 agent는 Bash를 거부합니다. 직접 Claude exact-argv launch는 RTRT bwrap 밖에서 Claude Code 공식 sandbox(`enabled=true`, `failIfUnavailable=true`, `allowUnsandboxedCommands=false`), strict network allowlist, project-only home-read exception, credential/environment scrub, strict permission-only RTRT MCP를 사용해 계속 지원됩니다. Claude Linux dependency가 없으면 fail closed하며 RTRT는 설치하지 않습니다. `socat`은 Claude Code host의 optional prerequisite일 뿐 bundle/install되지 않습니다. 일반 setup은 Ollama probe, embedding 자동 활성화, adapter 설치, `@ai-sdk/openai-compatible` provider 생성을 하지 않으며 operator가 Ollama를 명시적으로 제공·승인해야 합니다. HTTP MCP는 nonempty bearer auth가 필수이고 process/network tool은 명시적 HTTP opt-in이 필요하며 filesystem tool은 canonical project에 bound됩니다. Uninstall은 service/integration을 먼저 중지·제거한 뒤 binary/file을 제거하고 typed path/MCP entry는 symlink와 안전하지 않은 ownership/type 변경을 거부합니다.

- **RTRT 전용 OpenCode-to-Claude permission bridge 완료**: 직접 Claude CLI lane은 표준 `--permission-prompt-tool mcp__rtrt__permission_prompt`를 사용합니다. OpenCode setup은 전역 `~/.claude.json`을 읽거나 쓰거나 요구하지 않고 전역 Claude provenance hook도 설치하지 않으며, 기존 foreign/shared Claude MCP config를 건드리지 않습니다. 각 exact direct Claude invocation은 global/user/project setting source를 비활성화한 뒤 공식 sandbox policy와 exact `SessionStart` provenance hook을 담은 하나의 ephemeral strict settings object, 그리고 하나의 strict permission-only RTRT MCP config를 주입합니다. Broker는 `127.0.0.1` ephemeral port에 bind하며 invocation마다 random token/nonce와 부모 session/call identity를 사용합니다. `rtrt-mcp`는 Claude tool request의 제한된 field만 전달하고 raw prompt, credential, token, nonce, raw tool input을 자동 capture/persist하지 않습니다. OpenCode v2 native permission은 기존 project/global policy를 먼저 평가하고 필요할 때 native once/always/reject UI를 표시하며, always persistence는 OpenCode만 소유합니다. Approval은 wall-clock timeout 없이 native OpenCode처럼 decision 또는 lifecycle cancellation까지 기다립니다. Connect establishment만 짧게 제한하며 malformed/auth/connect failure, cancellation, disconnect, disposal은 기본 deny입니다. Standalone daemon/script/service, third-party plugin, 새 dependency는 없고 native Task inheritance는 바뀌지 않았습니다. Claude Code 2.1.219–2.1.221에서 support를 검증했고 설치된 OpenCode SDK contract는 1.18.11입니다(더 넓은 minimum compatibility는 주장하지 않음). OpenCode Claude CLI lane에서 `allowed-tools`, `bypassPermissions`, `dangerously-skip`은 거부합니다. Upgrade 후 OpenCode를 재시작하고 setup을 다시 실행해야 합니다.
- **Native Task permission은 기본적으로 host에서 상속**: OpenCode native Task worker는 별도 `[team.members.permissions]` block이 edit 또는 Bash 동작을 override하지 않는 한 OpenCode project/global permission을 상속합니다. Role은 routing label일 뿐이며 dashboard **Tools › Orchestration**에서 optional override를 편집할 수 있습니다. 기본 command-list allowlist는 없습니다. Task recursion, project 외부 directory, RTRT agent/team bridge는 고정 deny입니다. Claude CLI lane은 Claude Code settings, `permission-mode`, 표준 `--permission-prompt-tool mcp__rtrt__permission_prompt`를 함께 사용하며(`opus`는 `plan`, `sonnet`은 `acceptEdits`), OpenCode Claude CLI lane에서 `allowed-tools`, `bypassPermissions`, `dangerously-skip`은 거부합니다.
- **Runtime routing 문서 정정**: routing은 materialized RTRT config — member roles, tiers, `design_only_tiers`, `review_tier`, `leader_order`, siblings, fallbacks, `balance`, delegation, flags — 에서만 나옵니다. Preset은 편집 가능한 초기값(GPT manager, Codex-first hard tier, Claude plan/review roles, Kimi overflow)을 제공할 뿐이며, source manager policy는 architecture/security 등의 trigger나 model-name phase를 강제하지 않습니다. Quota 실패 시 같은 lane은 재시도하지 않고(0회), 현재 session에서 해당 lane을 건너뜁니다. 이후 실행 대상은 설정된 sibling/fallback policy가 결정합니다.
- **위임 transport와 provider invocation mode를 분리**: `delegation = "native"`는 OpenCode native Task, `delegation = "cli"`는 외부 CLI를 선택하며 현재 `target = "claude"`에서 `claude -p`를 호출합니다. 레거시 `shell`은 읽기 전용 호환 alias로 유지되고 serialization과 dashboard 저장은 `cli`를 사용합니다. Dashboard dropdown에는 **Native Task / CLI**가 표시됩니다. 이는 provider invocation mode인 `mode = "cli" | "api" | "auto"`와 별개입니다.
- **`opencode-go`와 Ollama Cloud의 별도 quota를 사용하는 설정 기반 OpenCode native agent** (`rtrt setup --agent opencode --apply`): 명시적 `opencode-lead` preset은 `openai/gpt-5.6-sol`을 manager로 사용하고, `opencode-go/glm-5.2` / `ollama/glm-5.2:cloud`, `opencode-go/kimi-k2.7-code` / `ollama/kimi-k2.7-code:cloud`, `opencode-go/kimi-k3` / `ollama/kimi-k3:cloud`의 symmetric sibling pair를 포함한 11개 lane을 구체화합니다. Codex, GLM, Kimi, Ollama lane은 OpenCode native Task를 사용합니다. Claude Opus/Sonnet은 유일한 external CLI 예외로, TeamConfig routing과 permission-mode 설정에 따라 `claude -p`를 직접 호출합니다. Setup은 `rtrt-manager`와 설정된 native `host_agent`를 관리 대상 global OpenCode agent file로 렌더링하고, 설정된 Claude CLI lane은 native agent 대신 dispatch entry로 유지하며 built-in discovery agent는 덮어쓰지 않고 재사용합니다. Native Task는 기본적으로 OpenCode project/global permission을 상속하고 optional member override만 edit/Bash 동작을 바꾸며, setup은 command allowlist를 만들지 않습니다. Provider/model, role, tier, mode, logical model, sibling/fallback link, host-agent 이름, delegation, implementation 허용 여부는 `[team]` 또는 dashboard에서 계속 편집할 수 있습니다. Setup 재실행은 RTRT-owned file과 JSON/JSONC field만 reconcile하고 team 비활성화 시 이를 surgical removal합니다. 외부 agent file, provider option, MCP entry, comment, 관련 없는 permission은 보존합니다. Claude CLI 인증과 필요한 subscription이 정상이어야 하며, 실패는 설정된 TeamConfig retry/fallback policy를 따릅니다. RTRT agent/team bridge나 중첩 OpenCode process/delegation은 사용하지 않습니다.
- **OpenCode native Task 구조화 provider-limit circuit breaker**: 기존 RTRT provenance plugin은 `rtrt-manager`가 만든 native child session만 추적합니다. OpenCode SDK의 `session.next.retried`에서 구조화된 `statusCode`가 `429` 또는 `529`이면 정확한 child를 즉시 interrupt하고, manager는 현재 session에서 해당 lane을 제외한 뒤 설정된 sibling, 다음 fallback 순서로 진행하며 같은 lane은 0회 재시도합니다. Free-text parsing과 wall-clock timeout은 사용하지 않으며 Claude permission approval, 일반 review, 무관한 agent, 일반 5xx, auth failure, unstructured error에는 영향을 주지 않습니다. 상태는 bounded in-memory로 유지하고 idle/delete/dispose 때 정리하며 prompt/body는 저장하지 않습니다. 구조화 status 처리, 정확한 child interrupt, routing 제외, cleanup, 비대상 case를 테스트합니다.

- **OpenCode용 공식 Claude Code 스테이터스라인 rate-limit bridge**: Claude Code는 갱신할 때 이미 공식 `rate_limits` object를 `rtrt statusline --rich`에 전달합니다. RTRT는 여기서 숫자형 `captured_at`, `used_percentage`, `resets_at`만 사용자 전용 local cache에 복사하며 credential, session/prompt 식별자, transcript data, token detail은 저장하지 않습니다. OpenCode는 network 없이 이 cache를 읽고 Claude 갱신 후 15분 이내이면서 해당 reset이 만료되지 않았을 때만 공식 `5H` / `WEEK` percentage와 reset countdown을 표시합니다. Stale, expired, absent, invalid window는 추정하지 않고 `N/A`로 유지합니다. `RTRT_CLAUDE_RATE_LIMIT_CACHE`는 기본 `~/.rtrt/statusline/claude-rate-limits.json` 경로를, `RTRT_CLAUDE_RATE_LIMIT_MAX_AGE_SEC`는 기본 900초 freshness gate를 재정의합니다. 이는 아래 최초 statusline 항목의 `WEEK N/A/not exposed` fallback을 대체하지만 session `COST`와 `CTX` semantics는 바꾸지 않습니다. `WEEK`은 Claude Code가 사용하는 동일한 공식 `rate_limits.seven_day` 값이며, RTRT rolling 7d ledger는 계속 관측된 local activity일 뿐 provider quota가 아닙니다. 문서화되지 않은 OAuth endpoint 직접 polling은 Claude credential을 취급해야 하고 지원되지 않는 ToS 및 response schema contract에 의존하므로 의도적으로 채택하지 않았습니다.

- **OpenCode용 영구 RTRT 스테이터스라인** (`rtrt setup --agent opencode`, `rtrt statusline --opencode`): setup이 관리 대상 `~/.config/opencode/tui/rtrt-statusline.tsx`와 `rtrt-statusline-core.mjs`를 설치하고 활성 `tui.json` / `tui.jsonc`의 `plugin` 배열에 RTRT tuple 하나를 병합합니다. 이 과정에서 외부 plugin과 관련 없는 config key는 보존합니다. TUI plugin은 전체 폭 `app_bottom`과 compact `session_prompt_right`(prompt-right)를 렌더링하고, 관련 OpenCode event 및 15초 주기로 갱신하며, argv-only child process로 CLI를 호출합니다. CLI는 version이 있는 compact JSON object 하나를 출력하고 stdin을 읽지 않으며, 명시적인 폭 tier를 사용합니다. 60 column 미만은 savings/style, 60-99는 project/headroom 추가, 100 이상은 Git/model/session/memory까지 허용합니다. Collector는 모두 local-only이며 기본 120 ms 전체 budget 안에서 동작합니다. 유효 config, 크기가 제한된 local ledger/cache, busy timeout 0의 read-only SQLite, 제한된 local Git status만 읽고 network request나 transcript scan은 수행하지 않습니다. 폭 tier상 수집 대상인 collector가 실패하거나 budget 때문에 생략되면 `degraded`에 기록하되 사용 가능한 segment는 유지합니다. Budget 만료 또는 stale Git fallback은 `stale`을 설정하고, 이후 plugin 호출이 실패하면 마지막 정상 line을 흐리게 표시하며 stale 표식을 붙입니다. 정상 결과가 한 번도 없었다면 `rtrt · n/a`를 표시합니다. OpenCode SDK 1.18.13은 현재 session model을 노출하며 plugin은 과거 message에서 추론하지 않고 안전한 `provider/model`을 argv-only `--model`로 snapshot에 전달합니다. Session 지표도 OpenCode state를 그대로 사용합니다. `COST`는 양수인 `Session.cost` 추정치를 `~$`, 0을 포함/무료/가격 미책정 중 무엇인지 구분할 수 없다는 뜻의 `$0?`, 값이 없으면 `N/A`로 표시합니다. `CTX`는 가장 최근에 완료된 assistant turn과 그 turn의 정확한 model context limit를 사용하며 overflow 시 100%를 넘는 값도 그대로 표시합니다. State는 `BUSY`, `RETRY`, `IDLE`로 명시합니다. SDK가 안정적인 account-level 5h 또는 weekly limit를 노출하지 않으므로 `WEEK`은 `N/A/not exposed`입니다. RTRT의 rolling 7d ledger는 관측된 local activity일 뿐 provider weekly limit와 별개이며, `WEEK`의 근거로 사용하거나 quota라고 부르면 안 됩니다. `rtrt uninstall --agent opencode --apply`는 RTRT TUI tuple과 인식 가능한 관리 file block만 제거하고 외부 plugin/key 및 수정됐거나 인식할 수 없는 file content를 보존하며, 기존의 다른 RTRT 관리 OpenCode 연동 항목도 제거합니다. TUI plugin은 process 시작 시 load되므로 setup 또는 uninstall 후 OpenCode를 재시작해야 합니다.
- **프로젝트 로컬 및 사용자 전용 fallback 임시 디렉터리** (`rtrt-core`, OpenCode plugin): `RTRT_TMP_DIR`가 모든 기본값을 재정의합니다. 그렇지 않으면 발견된 project는 `<main-linked-repository-root>/.rtrt/tmp`를 사용하고, project가 없는 실행은 공유 `<OS temp>/rtrt` 대신 OS temp 아래의 사용자 전용 디렉터리(Unix는 `<OS temp>/rtrt-<uid>`, 기타 플랫폼은 동등한 사용자 전용 디렉터리)를 사용합니다. Runtime scratch 후보는 symlink가 아닌 실제 디렉터리여야 하며, Unix에서는 소유자와 `0700` mode도 검증해 안전하지 않은 후보를 거부합니다. 발견된 project의 로컬 디렉터리 생성 실패는 OS temp로 조용히 빠져나가지 않고 오류가 됩니다. OpenCode plugin은 session별 `.rtrt/tmp/opencode/<session>`을 `TMPDIR`, `TEMP`, `TMP`에 지정합니다. 이는 child process의 temp 위치만 제어하며 현재 plugin SDK로는 OpenCode native Task의 내부 worktree root를 강제할 수 없습니다. 이미 `/tmp/opencode`를 사용하는 session은 OpenCode 재시작 전까지 바뀌지 않습니다.
- **OpenCode 세션 자동 캡처** (`rtrt-dashboard`): MCP 도구를 수동 호출할 때만 저장되던 OpenCode 기록을 이제 읽기 전용 SQLite watcher가 자동 수집합니다. 세션 디렉터리를 Git 루트 프로젝트로 귀속하고 완료된 사용자·assistant·subagent 텍스트에 session/parent/message 출처를 붙여 저장하며, 재시작 시 기존 본문은 중복 저장하지 않고 streaming 중인 assistant 메시지는 완료될 때까지 기다립니다. 비표준 DB는 `RTRT_OPENCODE_DB`로 지정할 수 있습니다. 명시적인 `/tmp/opencode/<project>-<lane>` 형태에서 실행한 외부 `claude -p` 기록은 임시 checkout이 사라진 뒤에도 이미 알려진 scope 프로젝트로 접어 별도 lane 프로젝트가 생기지 않게 합니다. Claude JSONL watcher도 SQLite 잠금으로 저장이 실패한 줄의 offset을 넘기지 않고 다음 polling에서 재시도해 기록 유실을 막습니다.
- **외부 Claude 세션의 부모 provenance를 경로 추정 대신 영구 저장** (`rtrt setup --agent opencode`, `rtrt-mcp`, `rtrt-providers`, `rtrt-memory`, `rtrt-dashboard`): 전역 OpenCode plugin이 invocation UUID를 만들고 부모 project/session/call, 활성 agent, cwd, worktree를 RTRT MCP 호출과 직접 shell 환경에 전달합니다. 각 exact direct `claude -p` invocation은 global/user/project setting source를 비활성화하고 exact `SessionStart` provenance hook을 담은 ephemeral strict settings object와 strict permission-only RTRT MCP config를 주입합니다. 기존 foreign/shared Claude MCP config는 무관하고 보존됩니다. RTRT가 실행하는 `claude -p`에는 명시적인 child `--session-id`도 부여하며 hook이 child ID와 부모 context를 새 schema-v8 `invocation_provenance` 테이블에 연결합니다. 최초 저장된 child session 소유자는 이후 resume가 바꿀 수 없고, capture는 transcript row와 함께 provenance metadata를 기록하며, MCP 자동 캡처는 전달된 부모 project를 fallback으로 사용할 수 있습니다. OpenCode setup은 writable project root 밖의 executable을 요구하므로 설치된 `~/.cargo/bin/rtrt`를 사용하고 project `target/` binary는 사용하지 않습니다. 설치 후 OpenCode 재시작이 필요합니다.
- **OpenCode-lead roster는 classic 호환 명시적 opt-in** (`rtrt team preset opencode-lead [--apply]`): 기본 실행은 dry-run이며 `--apply`는 관련 없는 설정을 유지하면서 global `[team]` section만 교체해 roster를 구체화합니다. Preset은 편집 가능한 초기값 — GPT manager, Codex-first hard tier, Claude plan/review roles, Kimi overflow — 을 제공합니다. Runtime routing은 materialized config의 roles, tiers, `design_only_tiers`, `review_tier`, `leader_order`, siblings, fallbacks, balance, delegation, flags를 읽으며 model name으로 task phase를 추론하거나 architecture/security 등의 trigger를 hardcode하지 않습니다. Dashboard/config 편집이 유효 policy를 바꾸고, setup 재실행이 generated host file을 reconcile합니다. Native Task / `@mention` spawn, 병렬성, session tree, host permission enforcement는 OpenCode가 소유하며 native delegation은 RTRT의 중첩 `opencode` CLI 실행을 허용하지 않습니다.
- **Claude CLI 결과를 구조화하고 Claude가 보고한 호출별 비용을 보존** (`InvokeOutcome`): RTRT는 Claude CLI 호출에 JSON 출력을 요청하고 `result`를 추출하며, 유한한 0 이상의 `total_cost_usd`를 optional `cost_usd`로 전달합니다. Cost가 없거나 잘못되면 `None`을 유지하고, 명시적 non-JSON 또는 malformed JSON이면 호출을 실패시키지 않고 plain 출력을 보존합니다. 이는 호출별 회계일 뿐이며 RTRT는 아직 team/model aggregate cost report를 제공하지 않습니다.
- **Worker return과 write isolation에 엄격하고 결정론적인 contract 추가** (`rtrt-orchestrator`): `WorkerReturn` validation은 수정 파일, test command/status, 설정된 제한 이내의 summary(기본 3줄), 보관된 상세 기록을 위한 non-empty `detail_ref`를 요구합니다. `WorkerResult`는 assigned lane과 actual lane의 fallback provenance도 검증합니다. 순수 isolation planner는 입력 순서를 보존하고 lexical write set이 겹치지 않는 안정적인 연속 wave를 만들며, 동일 경로나 부모/자식 경로 overlap에 참여한 모든 task를 전용 worktree 대상으로 표시합니다. 이는 integration primitive이며 native host runtime이 아닙니다. RTRT는 아직 OpenCode-native task를 schedule하거나 worktree를 자동 merge하지 않습니다.
