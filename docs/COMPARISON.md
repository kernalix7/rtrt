...
| FTS | Bundled BM25 + synonym expansion | SQLite FTS5 BM25 (v0.1), synonym layer planned |
| Embeddings | `all-MiniLM-L6-v2` default; Gemini / OpenAI / Voyage / Cohere optional | `all-MiniLM-L6-v2` planned (offline only at first) |
| Graph | Knowledge-graph entity matching | Reserved schema (`edges` table); not implemented in v0.1 |
| Recall | Reciprocal Rank Fusion across BM25 + vector + graph | BM25 today; hybrid planned for v0.2 |
