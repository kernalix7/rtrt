...
| FTS | 내장 BM25 + 동의어 확장 | SQLite FTS5 BM25 (v0.1), 동의어는 예정 |
| 임베딩 | `all-MiniLM-L6-v2` 기본; Gemini/OpenAI/Voyage/Cohere 선택 | `all-MiniLM-L6-v2` 예정(우선 오프라인만) |
| 그래프 | 지식 그래프 엔티티 매칭 | 스키마 예약(`edges`); v0.1 미구현 |
| 회수 | BM25 + 벡터 + 그래프 Reciprocal Rank Fusion | v0.1은 BM25, 하이브리드는 v0.2 |
