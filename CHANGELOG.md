...
- **`rtrt setup --agent opencode`**: wires the rtrt MCP server into opencode, the same way `--agent claude/cursor/windsurf/codex` already do, so opencode reads the same unified `~/.rtrt/memory.sqlite` memory store. Merges an `mcp.rtrt` entry (`type: "local"`, `command` as an array of the binary plus `--memory <store>`, `enabled: true`) into `~/.config/opencode/opencode.jsonc`, and writes the terse Output Optimizer rules block to opencode's global `~/.config/opencode/AGENTS.md`. Since the config is JSONC, a strict-JSON file (the common case) round-trips through a full parse/merge/re-serialize; a file that actually has comments or trailing commas falls back to a best-effort textual insert that never touches the rest of the document, so existing comments survive. Dry-run by default, `--apply` to write, `.bak` backup on first write, and idempotent on repeat runs — same contract as every other `rtrt setup` agent. `rtrt setup --agent opencode --plugin` stays unsupported (`--plugin` is Claude-only, same as Cursor/Windsurf/Codex).
- **Transcript watcher now captures genuine user prompts, not just assistant output** (dashboard): the watcher only ever stored assistant/teammate turns, so old/backfilled and subagent-session transcripts ended up with answers that have no matching question — user input was only ~3% of stored rows. It now also captures `user`-role transcript lines that carry real typed text: a plain-string `message.content`, or an array whose parts include `text` and NO `tool_result` (a tool_result is the harness echoing a tool call back through the same `user`-role channel, never something the user typed). Captured rows land as `user-prompt-submit` / `source_kind=main` — even inside a `/subagents/` transcript, since that line is the parent handing the subagent its task — using the same project/session/parent/agent resolution as the assistant path, and dedup against the live `UserPromptSubmit` hook via the existing `body_sha` check so nothing doubles up. Harness-injected synthetic prompts (background-task notifications, `<task-notification>` blocks, bare `task-id:`/`tool-use-id:` metadata) are skipped via `rtrt_memory::is_synthetic_prompt`, now shared between the CLI hook and the watcher instead of duplicated.
- **Memory role filter — Input vs Output** (dashboard Memory Timeline + Search): the user's own typed prompts (`user-prompt-submit` / `user-prompt-expansion`) are a tiny fraction of total memory rows next to everything an agent produces (assistant turns, teammate messages, stop/subagent-stop captures, …), so row-level views read as "only output" by default. `GET /api/memory/timeline` and `POST /api/memory/recall` now accept a `role` param (`input` | `output` | absent = all) that filters by this coarse kind-group, composing with the existing `source_kind` (main/subagent) filter rather than replacing it. The grouping itself is defined once, in `rtrt_memory::role` (`INPUT_KINDS`, `role_matches`, `role_sql_clause`), so SQL `WHERE` clauses and in-process post-filtering can never drift apart. The Timeline and Search views gained a 전체/입력/응답 (All/Input/Output) toggle next to the existing Main/Subagent filter; the Timeline choice persists across sessions (`localStorage`). Input rows carry a `🗣 입력` badge and a subtle left-accent so they're spottable even in the unfiltered "All" view. The memory map (cluster overview) is unaffected — isolating input there is a possible follow-up.
...
- **Orphaned capture-bucket cleanup** (dashboard project selector): a new conservative classifier (`rtrt_memory::is_capture_bucket_name`) recognizes only unambiguously machine-generated memory-bucket names — `agent-*`, `p<n>-*`, a bare 32-hex session id, or a `32hex-40hex` session-hash pair — for the rare case where automatic reattribution can never resolve a real project because its source transcript was deleted. `GET /api/projects` now excludes these (unless explicitly registered) and reports an honest `hidden_capture_buckets` / `hidden_capture_bucket_rows` count instead of silently dropping them; `GET /api/projects/hidden` lists them by name; `POST /api/projects/reassign` folds one bucket's rows into a real project (`MemoryStore::reassign_project`, a parameterized bulk `UPDATE` — no rows are ever deleted). The project modal surfaces the hidden count and a bucket → project "Fold in" picker. The transcript watcher (`transcripts.rs`) also now resolves a subagent row's project and parent session AT CAPTURE TIME (not just via the later reattribution pass) and, if that genuinely can't be resolved, falls back to a synthesized-but-classifiable bucket name instead of silently dropping the row — closing the gap that produces new orphans in the first place.
...
- **Auto-recall (`hook recall`) quality — self-echo, garbage embeddings, no relevance floor, legacy noise**: the `UserPromptSubmit` context injection had four compounding bugs that made it inject mostly noise. (1) *Self-recall*: `hook capture user-prompt-submit` runs before `hook recall` and had already saved the current prompt, so recall could return that same row as one of its own hits — the user's own words echoed back. Hits are now filtered against the current prompt by both `MemoryStore::body_sha` and trimmed-text equality. (2) *Semantic recall destroyed*: the hook embedded the sanitized FTS5 OR-join (`"router OR headroom OR failover"`) instead of the natural-language prompt, so the dense-vector leg compared against a keyword bag instead of real text. `MemoryStore::recall_hybrid_with` now takes the lexical query and the semantic text as two separate arguments — the BM25 leg gets the OR-join, the embedder gets the raw prompt — while `recall_hybrid` keeps its old single-argument behavior for MCP's `memory_recall` / `memory_smart_search`, which already passed raw query text. (3) *No relevance floor*: recall previously injected up to `limit` rows regardless of whether the prompt had anything to do with the project. `MemoryStore::distinctive_terms` now gates injection on a data-derived signal: a query term must be rare within the project (document frequency ≤ √(project rows), the same scaling used elsewhere for data-derived caps) AND — when the store holds more than one project — enriched relative to its own global (whole-store) baseline rate, so a foreign-language function word that merely looks rare in one project (because the rest of the store is written in a different language) no longer passes as "distinctive." A query with no distinctive term injects nothing at all (and skips the BM25/hybrid work entirely); a returned hit that shares no distinctive term with the prompt is also dropped, since dense-vector recall always returns its top-K nearest neighbours regardless of true similarity. (4) *Legacy synthetic noise*: ~1,500 pre-existing `user-prompt-submit` rows captured before #76 stopped saving harness-injected `task-id:`/`<task-notification>` events still surfaced in recall; hits are now filtered through the existing `rtrt_memory::is_synthetic_prompt`. `RTRT_RECALL_DEBUG=1` opt-in stderr logging was added to `hook recall` to show whether the hybrid gate passed and how long it took, without ever touching stdout (the channel that gets injected into the model).
...
- **Memory map top-level overview no longer dumps most of a project into one catch-all bubble**: on large projects the whole-project cluster overview (`GET /api/memory/graph?mode=overview`) could leave a single bubble holding the majority of all rows (observed: 70% in one lexical-basis project, 90% across the top two bubbles in a vector-basis project), so the map read as one undifferentiated mass with nothing to drill into. The per-bubble drill already had an anti-stall guard for this (re-cluster, then fall back through metadata facets); the top level now runs the same cascade on any bubble that reaches the same dominance bar, splitting it into facet-labeled sub-bubbles (session/time/kind) instead of a generic "(기타)" blob. Every member id is preserved (`size_sum` still equals the project total) and every resulting bubble still mints its own drill token.
